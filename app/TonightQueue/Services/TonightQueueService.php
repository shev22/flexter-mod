...
class TonightQueueService implements TonightQueueServiceInterface
{
    ...
    public function pickRandom(User $user, string $source = 'watchlist'): array
    {
        if ($source === 'queue') {
            $items = $this->forUser($user)['items'];
            ...
            return [
                'picked' => collect($items)->random(),
                'items' => $items,
            ];
        }

        $pool = $this->watchlistCandidates($user);

        $existing = collect($this->forUser($user)['items'])
            ->mapWithKeys(fn (array $item) => ["{$item['type']}:{$item['id']}" => true]);

        $candidates = $pool
            ->filter(fn (array $item) => ! $existing->has("{$item['type']}:{$item['id']}"))
            ->values();
        ...
        /** @var array<string, mixed> $pick */
        $pick = $candidates->random();
        $items = $this->add($user, $pick);
     ...
     */
    private function watchlistCandidates(User $user): Collection
    {
        $entries = WatchList::query()
            ->where('user_id', $user->id)
            ->whereIn('media_type', [Movie::class, Tv::class])
            ->with('media')
            ->get();

        return $entries
            ->map(function (WatchList $entry) {
                $media = $entry->media;
                ...
                $type = $media instanceof Tv ? 'tv' : 'movie';
                $card = MediaCardData::fromModel($media, true);
