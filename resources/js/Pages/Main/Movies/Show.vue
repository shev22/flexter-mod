        ...
        <div class="relative h-screen">
            <div class="absolute inset-0 overflow-hidden">
                ...
                <iframe
                    ...
                    :src="`https://www.youtube.com/embed/${movieData.trailer}?autoplay=1&loop=1&mute=1&controls=0&showinfo=0&modestbranding=1&rel=0&enablejsapi=1`"
                    frameborder="0"
            ...
            <ShowMediaHeader
                ...
                :subtitle="`${movieData.seasons} Seasons`"
                @play="showModal = true"
            ...
            <div class="absolute bottom-0 left-0 w-full px-[5rem] z-20 ">
                ...
                <Carousel :items-to-show="12" :wrap-around="true" :autoplay="2000" :transition="500" :pause-autoplay-on-hover="true">
                    <Slide v-for="show in movieData.related" :key="show.id">
...
const movieData = computed(() => props.movie.data);
const inWatchlist = ref(movieData.value.in_watchlist);

const embedUrl = computed(() => `https://vidsrc.to/embed/movie/${movieData.value.id}`);
const backdropPath = computed(() => movieData.value.backdrop_path ? `https://image.tmdb.org/t/p/original/${movieData.value.backdrop_path}` : 'https://via.placeholder.com/1200x600');

const getPoster = (path) => path ? path : 'https://via.placeholder.com/150x220';

const toggleWatchlist = () => {
    const routeName = inWatchlist.value ? 'movie.remove.watchlist' : 'movie.add.watchlist';
    router.post(route(routeName), { id: movieData.value.id }, {
        preserveScroll: true,
